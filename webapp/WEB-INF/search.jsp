...
<body>
	<div  style="background-image: url('https://wallpaperaccess.com/full/5327327.jpg'); height:2000px" >
		...
		<div class="d-flex justify-content-center" style="margin-bottom:-150px; margin-top:30px">	
			<h4 class="text-info"> Game Search for <span style="color:blue"><c:out value="${name }"/></span> </h4>

		</div>
			
		<div class="d-flex">
			<div  class="shadow p-3 mb-5 bg-body rounded" style="height: 500px; text-align: justify;width:15% ;  margin-top: 200px; margin-left:5%;margin-right:5%">			
				<br />
				<br />
			...
			<c:if test="${games.isEmpty()}">
				<div style="width:250px; height: 50px; margin-left:20%;text-align: center; margin-top:200px" class="shadow-lg p-3 mb-5 bg-body rounded">
						<p>No search Results</p>			
				</div>
			</c:if>
			<div class="d-flex flex-wrap" style="margin-top:200px">
				<c:forEach items="${games}" var="game">
					<div style="width:250px; height: 350px; margin:5px;text-align: center" class="shadow-lg p-3 mb-5 bg-body rounded">
						...
						<h6 class="text-info"><a href="/game/<c:out value='${game.game_id }'/>"><c:out value="${game.game_name}"/></a></h6>
					 	...
					 	<p class="text-danger">Release Date:<c:out value="${game.release_date }"/></p>
					 	<p class="text-warning">Platform: <span class="text-info"><c:out value="${game.platform }"/></span></p>
					</div>
				</c:forEach>
			</div>
		</div>	
